-- =====================================================================
-- 2026-09-22 — daily_signals: procedencia del sueño + minutos del reloj
-- (V-15 Smart Recovery · docs/product-vision-roadmap.md)
--
-- Hoy decidía si preguntar "¿Cuánto dormiste?" / "¿Entrenaste hoy?" leyendo
-- SOLO las tablas manuales, así que aunque el reloj ya había escrito la
-- noche o el entreno, la app volvía a preguntar. La view gana dos columnas
-- nuevas AL FINAL (mismo patrón que workout_source, cero cambio en las
-- columnas existentes):
--   · sleep_source    = 'manual' si hay sleep_logs con duración ese día,
--                       si no 'wearable' si el reloj trajo la noche, si no
--                       null. El sueño manual sigue ganando en sleep_minutes.
--   · workout_minutes = suma de duration_min de los workouts del reloj ese
--                       día (null si el entreno fue manual — nunca 0).
-- wearable_steps sigue fuera de la view (ingest-only, spec §1).
--
...
--
-- Revertir: re-correr 20260707200001_daily_signals_wearable_merge.sql.
-- =====================================================================
