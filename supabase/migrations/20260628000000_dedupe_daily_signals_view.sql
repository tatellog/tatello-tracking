-- daily_signals devolvía varias filas por (user, day) porque sleep_d, water_d,
-- rest_d y period_d no estaban agregadas a 1 fila/día → fan-out en los LEFT JOIN.
-- Esto rompía getDaySignals().maybeSingle() (ver un día desde Semana) e inflaba
-- conteos que cuentan por fila. Se agregan esas 4 sub-consultas a 1 fila/día.
-- Columnas de salida idénticas (CREATE OR REPLACE seguro).
CREATE OR REPLACE VIEW public.daily_signals AS
 WITH sleep_d AS (
